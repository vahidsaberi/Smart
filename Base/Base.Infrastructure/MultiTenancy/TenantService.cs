...
using Base.Application.Multitenancy;
...
{
    private readonly IMultiTenantStore<SmartTenantInfo> _tenantStore;
    ...
    public TenantService(
        IMultiTenantStore<SmartTenantInfo> tenantStore,
    ...
    public async Task<string> CreateAsync(CreateTenantRequest request, CancellationToken cancellationToken)
    {
        ...
        var tenant = new SmartTenantInfo(request.Id, request.Name, request.ConnectionString, request.AdminEmail, request.Issuer);
    ...
    private async Task<SmartTenantInfo> GetTenantInfoAsync(string id) =>
        await _tenantStore.TryGetAsync(id)
            ?? throw new NotFoundException(_t["{0} {1} Not Found.", typeof(SmartTenantInfo).Name, id]);
}