...
internal partial class UserService
{
    ...
    ///     If no user is found with that ObjectId, a new one is created and populated with the values from the
    ///     ClaimsPrincipal.
    ///     If a role claim is present in the principal, and the user is not yet in that roll, then the user is added to that
    ///     role.
    /// </summary>
    public async Task<string> GetOrCreateFromPrincipalAsync(ClaimsPrincipal principal)
    {
        ...
        var role = PrincipalExtensions.FindFirstValue(principal, ClaimTypes.Role);
        if (role != null &&
    ...
    public async Task<string> CreateAsync(CreateUserRequest request, string origin)
    {
        ...
        await _userManager.AddToRoleAsync(user, SmartRoles.Basic);
    ...
    public async Task UpdateAsync(UpdateUserRequest request, string userId)
    {
        ...
        {
            user.ImageUrl =
                await _fileStorage.UploadAsync<ApplicationUser>(request.Image, FileType.Image, new CancellationToken());
    ...
    {
        var email = PrincipalExtensions.FindFirstValue(principal, ClaimTypes.Upn);
        ...
        else
        {
            user = new ApplicationUser
            {
                ...
                FirstName = PrincipalExtensions.FindFirstValue(principal, ClaimTypes.GivenName),
                LastName = PrincipalExtensions.FindFirstValue(principal, ClaimTypes.Surname),
                Email = email,
