...
internal partial class UserService
{
    public async Task<List<string>> GetPermissionsAsync(string userId, CancellationToken cancellationToken)
    {
        ...
        foreach (var role in await _roleManager.Roles
            ...
            permissions.AddRange(await _db.RoleClaims
                .Where(rc => rc.RoleId == role.Id && rc.ClaimType == SmartClaims.Permission)
    ...
    public async Task<bool> HasPermissionAsync(string userId, string permission, CancellationToken cancellationToken)
    {
        var permissions = await _cache.GetOrSetAsync(
            _cacheKeys.GetCacheKey(SmartClaims.Permission, userId),
    ...
    {
        return _cache.RemoveAsync(_cacheKeys.GetCacheKey(SmartClaims.Permission, userId), cancellationToken);
    }
}