...
internal class TokenService : ITokenService
{
        ...
        if (_currentTenant.Id != MultitenancyConstants.Root.Id)
        {
            if (!_currentTenant.IsActive)
                throw new UnauthorizedException(
                    _t["Tenant is not Active. Please contact the Base.Identity.Application Administrator."]);

            if (DateTime.UtcNow > _currentTenant.ValidUpto)
                throw new UnauthorizedException(_t[
                    "Tenant Validity Has Expired. Please contact the Base.Identity.Application Administrator."]);
        }
    ...
    private IEnumerable<Claim> GetClaims(ApplicationUser user, string ipAddress)
    {
        return new List<Claim>
        {
            ...
            new(SmartClaims.Fullname, $"{user.FirstName} {user.LastName}"),
            new(ClaimTypes.Name, user.FirstName ?? string.Empty),
            new(ClaimTypes.Surname, user.LastName ?? string.Empty),
            new(SmartClaims.IpAddress, ipAddress),
            new(SmartClaims.Tenant, _currentTenant!.Id),
            new(SmartClaims.ImageUrl, user.ImageUrl ?? string.Empty),
