...
internal partial class UserService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (await _userManager.IsInRoleAsync(user, SmartRoles.Admin)
            && request.UserRoles.Any(a => !a.Enabled && a.RoleName == SmartRoles.Admin))
        {
            // Get count of users in Admin Role
            var adminCount = (await _userManager.GetUsersInRoleAsync(SmartRoles.Admin)).Count;
